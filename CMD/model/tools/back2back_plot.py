...
class bbplot(object):
    ...
    '''
    #
    def __init__(self, conf, model_file, tol = 20, tol_type = "ppm"):
        '''
        Constructor
        '''
        ...
        self.ion_terms = {'b':'n','y':'c','c':'n','z':'c'}
        self.ion_color = {'b':'#1E1EE5','y':'#E51E1E','c':'darkgreen','z':'#9370db'}
        self.config = conf
        self.ion_types = conf.ion_types
        
        self.model = load_model(model_file)
        self.mod_mode = False
        self.ion2vec = Ion2Vector(modini = self.config.modini, ion_types = self.ion_types, cleave_site_num = self.config.time_step, max_charge = self.config.max_ion_charge, prev = 1, next = 1, from_CTerm = self.config.from_CTerm)
    
    def fix_C57(self):
        AAMass_fix_C57()
    
    def __read_mod__(self, modfile = Common_Config().modini):
        f = open(modfile)
        lines = f.readlines()
        f.close()
        ...
        for charge in range(1, max_charge+1):
            for ntype in range(len(self.ion_types)):
                ...
                for i in range(len(target_ions)):
                    ...
                    if x > self.max_plot_mz or y > -self.min_plot_inten:
                        matched_inten.append(0)
                    else:
                        if self.ion_terms[ion_type] == 'c': ion_idx = len(target_ions)-i
                        else: ion_idx = i+1
                        plotax.plot( [x,x], [0, y], color=self.ion_color[ion_type], lw=2)
                        plotax.text( x, y + vmargin, '{}{}{}'.format(ion_type,ion_idx,'+'*charge), rotation = 90, color=self.ion_color[ion_type], horizontalalignment="center",verticalalignment=valign)
    ...
    def Plot_Real(self, plotax, ions, spec):
        ...
        plotax.vlines(xmz, [0]*len(yint), yint, color = 'lightgray')
                ...
                for i in range(len(peak_idx)):
                    ...
                    if idx != -1:
                        if peaks[idx][1] >= self.min_plot_inten:
                            ...
                            plotax.text( peaks[idx][0], peaks[idx][1] + vmargin, '{}{}{}'.format(ion_type,ion_idx,'+'*charge), rotation = 90, color=self.ion_color[ion_type], horizontalalignment="center",verticalalignment=valign)
        plotax.text(20,1.2,'x {:.2e}'.format(max_inten))
        return matched_inten
    ...
    def output_predict_with_iontype(self, peptide, modinfo, charge):
        ...
        pred_charge = charge if charge <= self.config.max_ion_charge else self.config.max_ion_charge
        output = {}
        for i in range(len(self.config.ion_types)):
            it = self.config.ion_types[i]
            for ch in range(1, pred_charge):
    ...
    def plot(self, peptide, modinfo, spec):
        raw_scan = '.'.join(spec.split('.')[:-4])
        if not raw_scan in self.pf2idx:
            print('no spec {} in pf2 file'.format(spec)) 
            return
        ...
        ax.text(max_plot_mz - 600, 1, '{} ({}+)\nPearson CC = {:.2f}'.format(peptide,self.spec_charge,PCC[0]), fontsize = 16)
        ...
        print('Pearson CC = {:.2f}'.format(PCC[0]))
        
    def show(self):
        plt.show()
    
    def SavePlot(self):
        print('SavePlot(), to be finished')
        
    def pcc_batch(self, psmlist):
        pccs = []
        for psm in psmlist:
            pcc = self.match_noplot(peptide = psm[1], modinfo = '', spec = psm[0])
